    ...
    subcheats: [
        {
            ...
            fn: function (params) {
                const searchVals = [];
                const queryX =
                    params.slice(1) && params.slice(1).length ? params.slice(1).join(" ").toLowerCase() : undefined;
...
async function setup() {
    ...
    try {
        // Added try block for detailed error catching within setup
...
function setupw3StuffProxy() {
    ...
    actorEvents345._customBlock_WorkbenchStuff = function (...argumentsList) {
        ...
        if (cheatState.w3.buildspd && t == "PlayerBuildSpd") {
            // multiply build rate (check config.js)
...
function setupPoingProxy() {
    ...
    const poingGeninfo = bEngine.gameAttributes.h.PixelHelperActor[23].behaviors.behaviors[0].script._GenINFO;
...
function setupHoopsMinigameProxy() {
    try {
        const hoopsBehavior = bEngine.getGameAttribute("PixelHelperActor")[21].behaviors.getBehavior("ActorEvents_510");
...
function setupDartsMinigameProxy() {
    try {
        const dartsBehavior = bEngine.getGameAttribute("PixelHelperActor")[21].behaviors.getBehavior("ActorEvents_510");
...
function setupMonumentProxy() {
    ...
    const wisdomAttempt = bEngine.getGameAttribute("PixelHelperActor")[25].getValue("ActorEvents_670", "_GenINFO");
    ...
    bEngine.getGameAttribute("PixelHelperActor")[25].setValue("ActorEvents_670", "_GenINFO", proxyWisdom);
}
...
function getOptionsListAccount() {
    try {
        if (
            bEngine &&
            bEngine.gameAttributes &&
            bEngine.gameAttributes.h &&
            bEngine.gameAttributes.h.OptionsListAccount
        ) {
