...
const { objToString, prepareConfigForJson, parseConfigFromJson, getDeepDiff } = require('../utils/helpers');
...
function setupApiRoutes(app, context, client, config) {
  ...
  app.post('/api/config/save', async (req, res) => {
    ...
    try {
      ...
      // 3. Compute diff against defaults to save only overrides
      const cheatConfigDiff = getDeepDiff(parsedUiCheatConfig, defaultConfig?.cheatConfig) || {};
      const injectorConfigDiff = getDeepDiff(uiInjectorConfig, defaultConfig?.injectorConfig) || {};
      ...
      // 4. Construct file content string with only the overrides
...
`;
      ...
      if (uiInjectorConfig) _.merge(injectorConfig, uiInjectorConfig);
